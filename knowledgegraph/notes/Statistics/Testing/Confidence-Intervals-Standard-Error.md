...
In some cases we can run into problems with this. For instance, suppose we are trying to estimate the mean accuracy $p$ of a system. The per-example accuracy is a Bernoulli random variable. The standard error is
$$
...
Wilson asks "which values of $p$ could have plausibly produced the observation?", and uses these values of $p$ as the confidence interval. A candidate for $p$ is plausible if the observed $\hat{p}$ sits within 1.96 standard deviations of $p$, **where the standard deviation is computed from $p$, not the estimate $\hat{p}$**.
