...
- x.add_(5) - adds 5 to each element in place (discouraged since it's harder to backprop)
...
y3 = torch.randn_like(y)  # Method3
torch.matmul(A, A.T, out=y3)
```

# Tensor and numpy arrays share underlying memory
```python
x = torch.ones(5)
x_np = x.numpy()
x += 5
